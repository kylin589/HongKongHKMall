...
{
      [UserAuthorize]
    public class HNavigationController : HKBaseController
    {
        private readonly IFloorCategoryService _floorCategoryService;
        private readonly ICategoryService _categoryService;

        public HNavigationController(IFloorCategoryService floorCategoryService, ICategoryService categoryService)
        {
        ...
        public JsonResult Delete(long? floorCategoryId)
        {
            ...
            if (floorCategoryId.HasValue)
            {
                ...
                if (result > 0)
                {
                    ...
                    resultModel.Messages = new List<string> { "Delete homepage NavInfo success" };
                }
                else
                {
                    resultModel.IsValid = false;
                    resultModel.Messages = new List<string> { "Delete homepage NavInfo failed" };
                }
            }
            else
            {
                resultModel.IsValid = false;
                resultModel.Messages = new List<string> { "Key ID of homepage NavInfo  error" };
            }
        ...
        public ActionResult Create(FloorCategoryModel model)
        {
            ...
            else
            {
                ...
                {
                    resultModel.IsValid = true;
                    resultModel.Messages = new List<string>() { "Add homepage NavInfo success" };
                }
                else
                {
                    resultModel.IsValid = false;
        ...
        /// <returns></returns>
        public JsonResult UpdatePlace(long? floorCategoryId, int rowId, int sortType, int PagedIndex, int PagedSize)
        {
            var resultModel = new ResultModel();
