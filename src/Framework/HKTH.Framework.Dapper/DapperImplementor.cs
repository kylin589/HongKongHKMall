...
namespace HKTH.Framework.Dapper
{
	...
	public class DapperImplementor : IDapperImplementor
	{
		...
		public void Insert<T>(IDbConnection connection, IEnumerable<T> entities, IDbTransaction transaction, int? commandTimeout) where T : class
		{
			...
			bool identityHasValue;
			string sql = SqlGenerator.Insert(classMap, entities.FirstOrDefault(), out identityHasValue);
		...
		public dynamic Insert<T>(IDbConnection connection, T entity, IDbTransaction transaction, int? commandTimeout) where T : class
		{
			...
			bool identityHasValue;
			string sql = SqlGenerator.Insert(classMap, entity, out identityHasValue);
			if (identityColumn != null && !identityHasValue)
			{
		...
		protected IPredicate GetKeyPredicate<T>(IClassMapper classMap, T entity) where T : class
		{
			...
			IList<IPredicate> predicates = (from field in whereFields select new FieldPredicate<T> {Not = false, Operator = Operator.Eq, PropertyName = field.Name, Value = field.PropertyInfo.GetValue(entity, null)}).Cast<IPredicate>().ToList();
