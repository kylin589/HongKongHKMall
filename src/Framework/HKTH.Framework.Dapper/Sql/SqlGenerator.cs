...
namespace HKTH.Framework.Dapper.Sql
{
	public interface ISqlGenerator
	{
		...
		string Insert(IClassMapper classMap, object entity, out bool identityHasValue);
	...
	public class SqlGeneratorImpl : ISqlGenerator
	{
		...
		public virtual string Count(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters)
		{
			...
			var sql = new StringBuilder(string.Format("SELECT COUNT(*) AS {0}Total{1} FROM {2}", Configuration.Dialect.OpenQuote, Configuration.Dialect.CloseQuote, GetTableName(classMap)));
			if (predicate != null)
			{
				sql.Append(" WHERE ").Append(predicate.GetSql(this, parameters));
			}
		...
		public virtual string Insert(IClassMapper classMap, object entity, out bool identityHasValue)
		{
			identityHasValue = false;

			var columns = new List<IPropertyMap>();
			foreach (IPropertyMap propertyMap in classMap.Properties)
			{
				if (!(propertyMap.Ignored || propertyMap.IsReadOnly || propertyMap.KeyType == KeyType.Identity))
				{
					columns.Add(propertyMap);
					continue;
				}

				if (propertyMap.KeyType == KeyType.Identity && entity != null)
				{
					PropertyInfo entityProperty = entity.GetType().GetProperty(propertyMap.Name);
					if (entityProperty != null)
					{
						object propertyValue = entityProperty.GetValue(entity);
						//若标识列有值，需要插入该值 而不是自动插入，所以返回该列
						if (propertyValue != null && propertyValue.ToString() != "0")
						{
							identityHasValue = true;
							columns.Add(propertyMap);
						}
					}
				}
			...
			string sql = string.Format("INSERT INTO {0} ({1}) VALUES ({2})", GetTableName(classMap), columnNames.AppendStrings(), parameters.AppendStrings());
		...
		public virtual string Update(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters)
		{
			...
			IEnumerable<IPropertyMap> columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.KeyType == KeyType.Identity));
			if (!columns.Any())
			{
				throw new ArgumentException("No columns were mapped.");
			}
			...
			return string.Format("UPDATE {0} SET {1} WHERE {2}", GetTableName(classMap), setSql.AppendStrings(), predicate.GetSql(this, parameters));
		}
		...
		{
			IEnumerable<string> columns = classMap.Properties.Where(p => !p.Ignored).Select(p => GetColumnName(classMap, p, true));
