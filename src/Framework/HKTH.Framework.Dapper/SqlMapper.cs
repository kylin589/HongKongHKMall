...
namespace HKTH.Framework.Dapper
{
	...
	static partial class SqlMapper
	{
		...
		private static Action<IDbCommand, bool> GetBindByName(Type commandType)
		{
			...
			MethodInfo setter;
			if (prop != null && prop.CanWrite && prop.PropertyType == typeof (bool) && ((indexers = prop.GetIndexParameters()) == null || indexers.Length == 0) && (setter = prop.GetSetMethod()) != null)
			{
		...
		public class Identity : IEquatable<Identity>
		{
			...
			{
				return other != null && gridIndex == other.gridIndex && type == other.type && sql == other.sql && commandType == other.commandType && connectionStringComparer.Equals(connectionString, other.connectionString) && parametersType == other.parametersType;
			}
...
this IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType
			...
			)
		{
			...
			if (multiExec != null && !(multiExec is string))
			{
				bool isFirst = true;
				int total = 0;
				using (IDbCommand cmd = SetupCommand(cnn, transaction, sql, null, null, commandTimeout, commandType))
				{
...
this IDbConnection cnn, string sql, object param, IDbTransaction transaction, bool buffered, int? commandTimeout, CommandType? commandType
		...
		{
			IEnumerable<T> data = QueryInternal<T>(cnn, sql, param as object, transaction, commandTimeout, commandType);
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType
		...
		private static IEnumerable<TReturn> MultiMap<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(this IDbConnection cnn, string sql, object map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType)
		{
			IEnumerable<TReturn> results = MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(cnn, sql, map, param, transaction, splitOn, commandTimeout, commandType, null, null);
		...
		private static CacheInfo GetCacheInfo(Identity identity)
		{
			...
			if (!TryGetQueryCache(identity, out info))
			{
				...
				if (identity.parametersType != null)
				{
...
#if !CSHARP30
					else if (typeof (IEnumerable<KeyValuePair<string, object>>).IsAssignableFrom(identity.parametersType) && typeof (IDynamicMetaObjectProvider).IsAssignableFrom(identity.parametersType))
					{
						info.ParamReader = (cmd, obj) =>
						{
					...
					SetQueryCache(identity, info); // fix memory issue （当参数化时才设置缓存）
				}
			}
			return info;
		}
		...
		private static Func<IDataReader, object> GetDeserializer(Type type, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing)
		{
			...
			if (!(typeMap.ContainsKey(type) || type.IsEnum || type.FullName == LinqBinary || (type.IsValueType && (underlyingType = Nullable.GetUnderlyingType(type)) != null && underlyingType.IsEnum)))
			{
		...
		private sealed class DapperRowMetaObject : DynamicMetaObject
		{
			...
			{
				var parameters = new[] {Expression.Constant(binder.Name), value.Expression};
		...
		private sealed class DapperRow : IDynamicMetaObjectProvider, IDictionary<string, object>
		{
		...
		public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused)
		{
			...
			foreach (PropertyInfo prop in props)
			{
				if (filterParams)
				{
					if (identity.sql.IndexOf("@" + prop.Name, StringComparison.InvariantCultureIgnoreCase) < 0 && identity.sql.IndexOf(":" + prop.Name, StringComparison.InvariantCultureIgnoreCase) < 0 && identity.sql.IndexOf("?" + prop.Name, StringComparison.InvariantCultureIgnoreCase) < 0)
					{
...
Type type, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing
			...
			)
		{
			...
			List<IMemberMap> members = (specializedConstructor != null ? names.Select(n => typeMap.GetConstructorParameter(specializedConstructor, n)) : names.Select(n => typeMap.GetMember(n))).ToList();
	...
	/// </summary>
			...
			{
				Identity identity = this.identity.ForGrid(typeof (TReturn), new[] {typeof (TFirst), typeof (TSecond), typeof (TThird), typeof (TFourth), typeof (TFifth), typeof (TSixth), typeof (TSeventh)}, gridIndex);
				try
				{
					foreach (TReturn r in
						SqlMapper.MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(null, null, func, null, null, splitOn, null, null, reader, identity))
					{
						yield return r;
					}
				}
...
#pragma warning restore 612, 618
	...
	partial class FeatureSupport
	{
		...
		/// </summary>
		private static readonly Dictionary<string, FeatureSupport> FeatureList = new Dictionary<string, FeatureSupport>(StringComparer.InvariantCultureIgnoreCase) {{"sqlserverconnection", new FeatureSupport {Arrays = false}}, {"npgsqlconnection", new FeatureSupport {Arrays = true}}};
	...
	sealed partial class DefaultTypeMap : SqlMapper.ITypeMap
	{
		...
		public ConstructorInfo FindConstructor(string[] names, Type[] types)
		{
			...
			foreach (ConstructorInfo ctor in
				constructors.OrderBy(c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1)).ThenBy(c => c.GetParameters().Length))
			{
				...
				for (; i < ctorParameters.Length; i++)
				{
					...
					if (unboxedType != types[i] && !(unboxedType.IsEnum && Enum.GetUnderlyingType(unboxedType) == types[i]) && !(unboxedType == typeof (char) && types[i] == typeof (string)))
						break;
				}
		...
		{
			PropertyInfo property = _properties.FirstOrDefault(p => string.Equals(p.Name, columnName, StringComparison.Ordinal)) ?? _properties.FirstOrDefault(p => string.Equals(p.Name, columnName, StringComparison.OrdinalIgnoreCase));

			if (property != null)
				return new SimpleMemberMap(columnName, property);

			FieldInfo field = _fields.FirstOrDefault(p => string.Equals(p.Name, columnName, StringComparison.Ordinal)) ?? _fields.FirstOrDefault(p => string.Equals(p.Name, columnName, StringComparison.OrdinalIgnoreCase));
		...
		{
			return propertyInfo.DeclaringType == type ? propertyInfo.GetSetMethod(true) : propertyInfo.DeclaringType.GetProperty(propertyInfo.Name, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).GetSetMethod(true);
		}
